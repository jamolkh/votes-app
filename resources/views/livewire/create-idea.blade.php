<form wire:submit.prevent="createIdea" action="#" method="POST" class=" space-y-4 px-4 py-6">
    ...
    <div>
        <select wire:model.defer="category" name="category_add" id="category_add" class="bg-gray-100 text-sm w-full border-none rounded-xl px-4 py-2">
            @error('category')
                <p class="text-red text-xs mt-1">
                {{$message}}
                </p>
    ...
    <div>
        <textarea wire:model.defer="description" name="idea" id="idea" cols="30" rows="4" class="w-full border-none bg-gray-100 rounded-xl placeholder-gray-700 text-sm px-4 py-2"
    ...
    <div class="flex items-center justify-between space-x-3">
        ...
        <button type="submit" class="flex items-center justify-center w-1/2 h-11 text-xs bg-theme
